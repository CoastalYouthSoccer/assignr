...
from availability import (get_arguments, get_referees)
...
class TestGetArguments(TestCase):
    def test_help(self):
        ...
        self.assertEqual(cm.output, [f"ERROR:availability:{USAGE}"])
    ...
    def test_invalid_start_date(self):
        ...
        self.assertEqual(cm.output,
                         ['ERROR:availability:Start Date value, 21/11/2023 is invalid'])
    ...
    def test_invalid_end_date(self):
        ...
        self.assertEqual(args, expected_args)
        self.assertEqual(cm.output,
                         ['ERROR:availability:End Date value, 11/32/2023 is invalid'])


class TestGetReferees(TestCase):
    @patch('builtins.open')
    @patch('availability.csv.reader')
    ...
    def test_missing_file_name(self):
        ...
        self.assertEqual(cm.output,
                         ['ERROR:availability:FILE_NAME environment variable not provided'])
    ...
    def test_file_not_found(self):
        with self.assertLogs(level='INFO') as cm:
            results = get_referees()
        self.assertEqual(cm.output,
                         ['ERROR:availability:valid_file.csv Not Found!'])
        self.assertEqual(results, [])
